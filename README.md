...
O IndexedDB guarda dados cifrados separados por identidade (AES-GCM; chave derivada da credencial com PBKDF2). Atualizar a página mantém o acesso e a rota na mesma aba por até 12 horas, limitado pela validade do token, e reabre o cofre preparado inclusive offline. Com conexão, o token é validado antes de retomar o espaço. Uma cápsula cifrada separada guarda o acesso; sua chave aleatória fica no sessionStorage. Após login online, a cápsula não guarda PIN/senha. No login exclusivamente offline, guarda a credencial cifrada até obter um token na reconexão. **Sair ou trocar perfil** remove essa restauração e preserva os rascunhos. Uma trava entre abas evita gravações concorrentes do mesmo perfil. O acesso online temporário não guarda rascunhos ao fechar/recarregar. Veja [ciclo de vida, privacidade e limites](../docs/decisions/0008-restauracao-de-acesso.md).
